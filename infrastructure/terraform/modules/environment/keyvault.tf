...
resource "random_password" "sqldb_ci_cd_user_password" {
  length           = 16
  special          = true
  override_special = "!#*()-_+?"
}
...
resource "azurerm_key_vault_secret" "sqldb_connection_string_products" {
  name         = "sqldb-connectionstring"
  key_vault_id = azurerm_key_vault.this.id
  value        = "Server=tcp:${azurerm_mssql_server.server.fully_qualified_domain_name},1433;Initial Catalog=${azurerm_mssql_database.db.name};Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;Authentication=\"Active Directory Default\""
}
